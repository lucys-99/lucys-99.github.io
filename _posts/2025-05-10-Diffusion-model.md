...
* **Definition:** An IAM role is an IAM identity that you can create in your account with specific permissions. It's similar to an IAM user, but it's not uniquely associated with one person. Instead, it's intended to be *assumed* by anyone who needs it.
* **Characteristics:**
    ...
    * A *trust policy* is attached to a role, defining which principals (users, AWS services, or accounts) are allowed to assume the role.
    * An *identity-based policy* (or policies) is also attached to the role, defining what the assumed role can do.
* **Use Cases:**
